 ...
 */
class ListesPersonnesPOR extends Dav\Collection {
  private $porid;
  private $name;
  private $children;
  private $auth;
  ...
  function __construct ($auth, $base, $porid, $ent_code) {
    $this->auth = $auth;
    $this->base = $base;
    $this->porid = $porid;
  ...
  function getChildren () {
    if ($this->children === NULL) {
      $toms = $this->base->meta_topmenu_liste_contenant_type ($this->auth->token, $this->porid, $this->ent_code);
      $this->children = array ();
      foreach ($toms as $tom) {
