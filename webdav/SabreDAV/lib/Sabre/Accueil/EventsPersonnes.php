 ...
 *  - la liste des menus contenant au moins une vue evenements
 */
class EventsPersonnes extends DAV\Collection {
  private $auth;
  private $children;
  ...
  function getChildren() {
    if ($this->children === NULL) {
      ...
      if (count ($por_ids) == 1) {
	$toms = $this->base->meta_topmenu_events ($this->auth->token, $por_ids[0]);
	foreach ($toms as $tom) {
	  $this->children[] = new EventsPersonnesTOM ($this->auth, $this->base, $tom['tom_id']);
	}
      } else {
	foreach ($por_ids as $por_id) {
	  $this->children[] = new EventsPersonnesPOR ($this->auth, $this->base, $por_id);
	}
      }
    }
